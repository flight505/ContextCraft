...
try {
  createDOMPurify = tryLoadModule('dompurify');
  const jsdom = tryLoadModule('jsdom');
  JSDOM = jsdom.JSDOM;
...
// Load ignore module with resilience
let ignore;
try {
  ignore = tryLoadModule("ignore");
...
// Add require for the new TreeSitter utilities with improved error handling
let mainProcessCompressCode = null;
let mainProcessRemoveComments = null;

// Helper function to try loading a module from multiple possible paths
const tryRequireFromPaths = (modulePath) => {
  const possiblePaths = [
    // For resources path (extraResources)
    path.join(process.resourcesPath, 'node_modules/tree-sitter'),
    // Standard relative path
    modulePath,
    // From app path
    path.join(app.getAppPath(), modulePath),
    // Absolute path for packaged app
    path.join(process.resourcesPath, 'app.asar', modulePath),
    // For unpacked resources
    path.join(process.resourcesPath, 'app.asar.unpacked', modulePath),
  ];
  ...
  for (const tryPath of possiblePaths) {
    try {
      ...
      console.log(`Failed to load from ${tryPath}`);
      // Continue to next path
    }
  }
  
  throw new Error(`Failed to load module from any location: ${modulePath}`);
};

try {
  // Try to load tree-sitter utils from multiple possible locations
  const treeSitterUtils = tryLoadModule('./utils/treeSitterUtils');
  mainProcessCompressCode = treeSitterUtils.mainProcessCompressCode;
  mainProcessRemoveComments = treeSitterUtils.mainProcessRemoveComments;
  console.log("Successfully loaded TreeSitter utilities");
} catch (err) {
  console.error("Failed to load TreeSitter utilities:", err.message);
  // Create no-op fallbacks for the functions
  mainProcessCompressCode = (source) => {
    console.warn("TreeSitter compressCode called but not available");
    return source; // Return original source as fallback
  };
  mainProcessRemoveComments = (source) => {
    console.warn("TreeSitter removeComments called but not available");
    return source; // Return original source as fallback
  };
}
...
let tiktoken;
try {
  tiktoken = tryLoadModule("tiktoken");
...
// Add diagnostic function for path resolution
function logPathDiagnostics() {
  ...
  console.log('Checking dist directory existence:');
  for (const distPath of possibleDistPaths) {
    try {
      const exists = fs.existsSync(distPath);
      console.log(`- ${distPath}: ${exists ? 'EXISTS' : 'NOT FOUND'}`);
      ...
      console.error(`- ${distPath}: ERROR - ${err.message}`);
    }
  }
  console.log('==================================');
}
...
function createWindow() {
  ...
  // Log diagnostic info during startup
  logPathDiagnostics();
  ...
  if (process.env.ELECTRON_START_URL) {
    ...
    if (isPackaged) {
      ...
      // For packaged apps, more reliable to use path.resolve with __dirname
      // This ensures proper asar path resolution
      startUrl = url.format({
        pathname: path.resolve(__dirname, '../dist/index.html'),
        protocol: 'file:',
        slashes: true
      });
      console.log(`Packaged app path resolved to: ${path.resolve(__dirname, '../dist/index.html')}`);
      
      // Fallback path to try if the app fails to load
      const fallbackPath = path.join(process.resourcesPath, 'app.asar/dist/index.html');
      console.log(`Fallback path if needed: ${fallbackPath}`);
    } else {
      // Not packaged, but still in production mode
      startUrl = url.format({
        pathname: path.resolve(__dirname, '../dist/index.html'),
        protocol: 'file:',
        slashes: true
  ...
  mainWindow.webContents.on(
    ...
    (event, errorCode, errorDescription, validatedURL, isMainFrame) => {
      // Log only for the main frame failures
      if (isMainFrame) {
        console.error(`FAILED TO LOAD URL: ${validatedURL}`);
        console.error(`Error Code: ${errorCode}`);
        console.error(`Description: ${errorDescription}`);
        // Add more context if available
        console.error(`Is Packaged App: ${app.isPackaged}`);
        console.error(`Resources Path: ${process.resourcesPath}`);
        logPathDiagnostics(); // Re-run diagnostics on failure

        // Retry with an explicit file URL if in production
        if (!process.env.ELECTRON_START_URL) {
          // Try multiple fallback paths in order
          const fallbackPaths = [
            // Path 1: Using process.resourcesPath (most reliable in packaged app)
            path.join(process.resourcesPath, 'app.asar/dist/index.html'),
            
            // Path 2: Using direct path to Applications folder (specific to macOS)
            app.isPackaged && process.platform === 'darwin' 
              ? '/Applications/ContextCraft.app/Contents/Resources/app.asar/dist/index.html' 
              : null,
              
            // Path 3: Using app.getAppPath() 
            path.join(app.getAppPath(), 'dist/index.html'),
            
            // Path 4: Original fallback as last resort
            app.isPackaged 
              ? path.join(process.resourcesPath, 'app.asar/dist/index.html') 
          ...
          const tryNextFallback = () => {
            if (fallbackIndex >= fallbackPaths.length) {
              console.error('All fallback paths failed to load. Cannot recover.');
              return;
            }
            
            const currentPath = fallbackPaths[fallbackIndex++];
            console.log(`Trying fallback path ${fallbackIndex}/${fallbackPaths.length}: ${currentPath}`);
            ...
            // Set up one-time handler for next failure
            if (fallbackIndex < fallbackPaths.length) {
        ...
        console.warn(`Failed to load non-main frame: ${validatedURL} (${errorCode})`);
      }
    },
  );
...
app.whenReady().then(() => {
  console.log('App ready. Verifying critical modules...');
  ...
  try {
    ...
    console.error('FATAL: Failed to load critical module:', moduleError);
    
    // Show error dialog but continue anyway - don't exit
    // This allows the app to at least try to start even if a module is missing
    dialog.showErrorBox('Module Loading Warning', 
      `Some required modules could not be loaded. Some features may not work correctly.\nError: ${moduleError.message}`);
    
    // Continue with app startup despite the error
    createWindow();
  }
