...
import { Plus, X, FileText, Copy, Minimize2 } from "lucide-react";
...
const FileCard = ({
  ...
  const { name, path: filePath, tokenCount, content, isCompressed } = file;
    ...
    >
      <CardContent className={styles.fileCardContent}>
        <div className={styles.fileCardHeader}>
          ...
          {isCompressed && (
            <div className={styles.compressedIcon} title="Function bodies removed">
              <Minimize2 size={10} />
            </div>
          )}
        </div>
        ...
        <div className={styles.fileCardActions}>
          <Button
            ...
            startIcon={isSelected ? <X size={12} /> : <Plus size={12} />}
          ...
          <Button
            ...
            startIcon={copied ? <Copy size={12} className={styles.copySuccess} /> : <Copy size={12} />}
            className={styles.fileCardAction}
          />
        </div>
      </CardContent>
