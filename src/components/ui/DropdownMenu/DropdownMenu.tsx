...
export interface DropdownMenuProps {
  ...
  /** Optional function to render custom content at the top of the menu */
...
export const DropdownMenu: React.FC<DropdownMenuProps> = ({
  ...
  // Update position and focus first item when opening
  useEffect(() => {
    if (isOpen) {
      updatePosition();
        ...
        if (!isOpen) {
          ...
          setIsOpen(true);
            ...
            if (menuItems && menuItems.length > 0) {
                e.preventDefault();
                const targetIndex = e.key === 'ArrowUp' ? menuItems.length - 1 : 0;
          ...
          {options.map((option, index) => {
            ...
            return (
              <div
                ...
                className={`${styles.dropdownItem} ${option.disabled ? styles.disabled : ''} ${isSelected ? styles.selected : ''}`}
